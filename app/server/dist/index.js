...
async function readRawTaskQueue() {
    try {
        const raw = await readFile(QUEUE_PATH, 'utf-8');
        const parsed = parseYaml(raw);
        return parsed?.list ?? [];
...
async function readContextRegistry() {
    try {
...
async function writeContextRegistry(entries) {
    await writeFile(CONTEXT_REGISTRY_PATH, stringifyYaml({ context: entries }), 'utf-8');
}
async function readActionRegistry() {
    try {
...
async function writeActionRegistry(entries) {
...
function normalizeActionStatus(status, target) {
...
function normalizeActionEntry(input, id) {
    ...
    const target = { kind, location, ...(cwd ? { cwd } : {}) };
    const action = {
        ...input,
        id,
...
async function syncActionArtifacts(action) {
    if (!action.id)
        return;
...
function describeContextSource(source) {
    ...
    if (typeof source.editable === 'boolean')
        parts.push(`  Editable: ${source.editable ? 'yes' : 'no'}`);
    if (source.default_action_id)
        parts.push(`  Default action: ${source.default_action_id}`);
    if (source.description)
        parts.push(`  ${source.description}`);
    if (source.path)
        parts.push(`  Path: ${source.path}`);
    if (source.provider)
        parts.push(`  Provider: ${source.provider}`);
    if (source.origin) {
    ...
    if (Array.isArray(source.artifacts) && source.artifacts.length > 0) {
        parts.push(`  Artifacts: ${source.artifacts.map(String).join(', ')}`);
    }
    return parts.join('\n');
}
function describeAction(action) {
    ...
    if (action.context_id)
        parts.push(`  Linked context: ${action.context_id}`);
    if (action.auto_created)
        parts.push('  Auto-created: yes');
    if (action.target?.kind)
        parts.push(`  Target kind: ${action.target.kind}`);
    if (action.target?.location)
        parts.push(`  Target location: ${action.target.location}`);
    if (action.target?.cwd)
        parts.push(`  Target cwd: ${action.target.cwd}`);
...
function collectActiveActionPaths(actions) {
    const paths = new Set();
...
async function loadContextBundle(source) {
    if (!source.path || typeof source.path !== 'string')
        return describeContextSource(source);
    const candidates = [
...
async function loadLegacyContextRef(ref) {
...
async function resolveTaskContext(task, contextRegistry) {
    const sections = [];
    const matchedIds = [];
    const missing = [];
...
const taskRunRegistry = new Map();
async function streamAgentExecution(c, params) {
    return streamSSE(c, async (stream) => {
        const { taskId } = params;
        if (taskId)
            taskRunRegistry.set(taskId, { timestamp: Date.now() });
        try {
...
app.patch('/api/context/:id', async (c) => {
    ...
    const body = await c.req.json();
    try {
        const entries = (await readContextRegistry()).map(e => e.id === id ? { ...e, ...body, id } : e);
        await writeContextRegistry(entries);
        return c.json({ ok: true });
    }
    catch (err) {
...
app.delete('/api/context/:id', async (c) => {
    ...
    try {
        ...
        const actions = (await readActionRegistry()).map(action => action.context_id === id && action.auto_created
...
app.delete('/api/actions/:id', async (c) => {
    ...
    try {
        ...
        const contexts = (await readContextRegistry()).map(context => context.default_action_id === id
...
app.get('/api/files/*', async (c) => {
    ...
    if (!fullPath.startsWith(OPS_DIR))
...
app.post('/api/agent/start', async (c) => {
    ...
    if (task) {
        ...
        if (task.context?.length)
            taskInfo += `Context refs: ${task.context.join(', ')}\n`;
        if (task.actions?.length)
            taskInfo += `Action refs: ${task.actions.join(', ')}\n`;
        if (task.depends_on?.length)
            taskInfo += `Depends on: ${task.depends_on.join(', ')}\n`;
    ...
    try {
        await updateTaskStatus(body.taskId, 'in_progress');
    }
    catch { /* no queue */ }
...
app.post('/api/scheduler/start', async (c) => {
    ...
    if (!thoughtsText)
        return c.json({ error: 'thoughts is required' }, 400);
    const contextRegistry = await readContextRegistry();
    const contextLines = contextRegistry.map(describeContextSource);
    ...
    try {
        ...
        if (existing.length > 0) {
            existingTasksText = existing.map(t => `- ${t.id}: ${t.title} [${t.status}]${t.priority ? ` (${t.priority})` : ''}${t.description ? ` — ${String(t.description).slice(0, 100)}` : ''}`).join('\n');
        }
        const nums = existing
